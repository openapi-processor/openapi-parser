...
class UriSpec : StringSpec({
    ...
    "provides uri format as annotation" {
        val uri = Uri(settings)

        val schema = JsonSchemaObject(mapOf("format" to "uri"), context)
        val instance = JsonInstance("any")
        val step = SchemaStep(schema, instance)

        uri.validate(schema, instance, step)

        step.getAnnotations(Keywords.FORMAT) shouldHaveSingleElement {a -> a.value == "uri"}
    }

    "provides uri-reference format as annotation" {
        val uri = UriReference(settings)

        val schema = JsonSchemaObject(mapOf("format" to "uri-reference"), context)
        val instance = JsonInstance("any")
        val step = SchemaStep(schema, instance)

        uri.validate(schema, instance, step)

        step.getAnnotations(Keywords.FORMAT) shouldHaveSingleElement {a -> a.value == "uri-reference"}
    }
})
