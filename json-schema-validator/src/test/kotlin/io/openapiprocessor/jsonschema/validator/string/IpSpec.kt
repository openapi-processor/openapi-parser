...
class IpSpec : StringSpec({
    ...
    "provides format ipv4 as annotation" {
        val ip = IpV4(settings)

        val schema = JsonSchemaObject(mapOf("format" to "ipv4"), context)
        val instance = JsonInstance("any")
        val step = SchemaStep(schema, instance)

        ip.validate(schema, instance, step)

        step.getAnnotations(Keywords.FORMAT) shouldHaveSingleElement {a -> a.value == "ipv4"}
    }

    "provides format ipv6 as annotation" {
        val ip = IpV6(settings)

        val schema = JsonSchemaObject(mapOf("format" to "ipv6"), context)
        val instance = JsonInstance("any")
        val step = SchemaStep(schema, instance)

        ip.validate(schema, instance, step)

        step.getAnnotations(Keywords.FORMAT) shouldHaveSingleElement {a -> a.value == "ipv6"}
    }
})
