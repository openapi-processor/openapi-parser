...
class UuidSpec : StringSpec({
    ...
    "provides format as annotation" {
        val uuid = Uuid(settings)

        val schema = JsonSchemaObject(mapOf("format" to "uuid"), context)
        val instance = JsonInstance("any")
        val step = SchemaStep(schema, instance)

        uuid.validate(schema, instance, step)

        step.getAnnotations(Keywords.FORMAT) shouldHaveSingleElement {a -> a.value == "uuid"}
    }
})
