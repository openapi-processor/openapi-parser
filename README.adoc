...
A link:{openapi}[OpenAPI] 3.2, 3.1, 3.0 parser with validation and pluggable document reader & json/yaml converter.
...
image:https://img.shields.io/badge/OpenAPI-3.2%2C_3.1%2C_3.0-%2300c000?label=OpenAPI[Static Badge]
...
=== link:{converter-jackson}3[io-jackson3]

link:{central-search}[image:{jackson-central-badge}[]]

json/yaml `Reader`, `Writer` & `Converter` based on link:{jackson}[Jackson 3].

=== link:{converter-jackson}[io-jackson]

link:{central-search}[image:{jackson-central-badge}[]]

json/yaml `Reader`, `Writer` & `Converter` based on link:{jackson}[Jackson 2].
